...
### AttributeError: 'NoneType' object has no attribute 'config' 에러를 수정하기 위해 ###
### huggingface transformers로부터 모듈 불러오기 ###
from transformers import BertModel, BertPreTrainedModel

class FocalLoss(nn.Module):
    def __init__(self, alpha=1.0, gamma=2, ignore_index=-1, size_average=True):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, inputs, targets, alpha=1):
        ...
        # inputs = inputs[(targets != self.ignore_index).nonzero().squeeze(-1)]
        # targets = targets[(targets != self.ignore_index).nonzero().squeeze(-1)]
        inputs = inputs[(targets != self.ignore_index).nonzero(as_tuple=False).squeeze(-1)]
        targets = targets[(targets != self.ignore_index).nonzero(as_tuple=False).squeeze(-1)]
        ########################################################################
        ...
        # focal
        batch_loss = -alpha * (torch.pow((1 - pt), self.gamma)) * torch.log(pt)
        # powself2
        #batch_loss = -alpha * (torch.pow((1 - pt), self.gamma/pt)) * torch.log(pt)
        # powself1
        #batch_loss = -alpha * (torch.pow((1 - pt), self.gamma * (1 - pt))) * torch.log(pt)
...
class BertForUtteranceEncoding(BertPreTrainedModel):
    ...
    ##################################### 디버깅 ############################################
    # 에러 : TypeError: embedding(): argument 'indices' (position 2) must be Tensor, not bool #
    def forward(self, input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False):
        return self.bert(
            input_ids=input_ids, 
...
class BeliefTracker(nn.Module):
    def __init__(self, args, num_labels, device):
        ...
        # )
        #######################################################################
        ### huggingface tranformers의 BertModel을 사용하였기 때문에 코드 수정
        self.utterance_encoder = BertForUtteranceEncoding.from_pretrained(
            args.bert_dir
        )
        #######################################################################
        ...
        ### Classifier
        if args.focal:
            self.nll = FocalLoss(0.25, args.gamma, ignore_index=-1, size_average=True)
            self.eval_nll = CrossEntropyLoss(ignore_index=-1)
        else:
    ...
    def initialize_slot_value_lookup(self, label_ids, slot_ids):
        ...
        ################################ 디버깅 ###################################
        ### 에러 : TypeError: embedding(): argument 'indices' (position 2) must be Tensor, not bool ###
        slot_type_ids = torch.zeros(slot_ids.size(), dtype=torch.long).to(slot_ids.device)
        ##############################################################################
        ...
        for s, label_id in enumerate(label_ids):
            ...
            ################################ 디버깅 ###################################
            ### 에러 : TypeError: embedding(): argument 'indices' (position 2) must be Tensor, not bool ###
            label_type_ids = torch.zeros(label_id.size(), dtype=torch.long).to(label_id.device)
            ##############################################################################
    ...
    def forward(self, input_ids, input_len, labels, n_gpu=1, target_slot=None, slot_alpha=None):
        ...
        hidden, _ = self.utterance_encoder(input_ids.view(-1, max_seq_length),
                                           token_type_ids.view(-1, max_seq_length),
                                           attention_mask.view(-1, max_seq_length),
                                           output_all_encoded_layers=False)
        hidden = torch.mul(hidden, attention_mask.view(-1, max_seq_length, 1).expand(hidden.size()).float())
        ...
        hidden = self.attn(self.query_word_layer_norm(hid_slot), hidden, hidden, mask=attention_mask.view(-1, 1, max_seq_length).repeat(slot_dim, 1, 1))
        ...
        # NBT
        ########################################################################### 디버깅 ###########################################################################
        ### 에러 : Warning: masked_fill_ received a mask with dtype torch.uint8, this behavior is now deprecated,please use a mask with dtype torch.bool instead. ###
        # turn_mask = torch.Tensor(ds, ts, ts).byte().to(self.device)
        turn_mask = torch.Tensor(ds, ts, ts).bool().to(self.device)
        ###############################################################################################################################################################
        ...
        hidden = self.attn2(self.query_sent_layer_norm(hid_slot).view(-1, ts, self.bert_output_dim), hidden, hidden, mask=turn_mask)
        ...
        for s, slot_id in enumerate(target_slot): ## note: target_slot is successive
            ...
            if labels is not None:
                ...
                if self.training is True:
                    if slot_alpha is not None:
                        _loss = self.nll(_dist.view(ds*ts, -1), labels[:,:,s].view(-1), alpha=slot_alpha[s] if slot_alpha is not None else 1)
                    else:
                        _loss = self.nll(_dist.view(ds*ts, -1), labels[:,:,s].view(-1))
                else:
                    _loss = self.eval_nll(_dist.view(ds*ts, -1), labels[:,:,s].view(-1))
