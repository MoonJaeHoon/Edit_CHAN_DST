...
class Processor(DataProcessor):
    ...
    def __init__(self, config):
        ...
        ################## WOS 데이터 경로도 추가 ##################
        # WOZ2.0 dataset
        if "data/woz" in config.data_dir or "data/wos" in config.data_dir:
...
def main():
    ...
    parser.add_argument("--data_dir",
                        ...
                        default="data/multiwoz-update",
                        type=str,
                        # required=True,
                        help="The input data dir. Should contain the .tsv files (or other data files) for the task.")
    parser.add_argument("--bert_model", default='bert-base-uncased', type=str, required=False,
                        help="사실, huggingface의 pretrained 로부터 tokenizer를 불러오기 때문에 필요없어진 argument이다"
    ...
    parser.add_argument("--tf_dir",
                        # default='tensorboard',
                        default='tensorboard/multiwos/wos',
                        type=str,
                        # required=False,
    ...
    if not args.do_not_use_tensorboard:
        ...
        summary_writer = SummaryWriter("./%s" % (args.tf_dir))
    else:
        summary_writer = None
    ...
    if args.local_rank == -1 or args.no_cuda:
        ...
        ############################### 디버깅 ################################
        # 에러 : AttributeError: 'BeliefTracker' object has no attribute 'module'
        n_gpu = 1
        #######################################################################
    else:
    ...
    if args.do_train:
        ...
        #train_data = TensorDataset(all_input_ids, all_input_len, all_label_ids, all_update)
        if args.local_rank == -1:
            train_sampler = RandomSampler(train_dataset)
        else:
            train_sampler = DistributedSampler(train_dataset)

        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, drop_last=True, num_workers=8, collate_fn=lambda x: collate_fn(x))
        ...
        #dev_data = TensorDataset(all_input_ids_dev, all_input_len_dev, all_label_ids_dev, all_update_dev)
